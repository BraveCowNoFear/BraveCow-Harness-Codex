...
只要一轮课程讲到按钮、菜单、模式、输入位置或点击动作，就必须先在同一条回复中发送对应图片，再写口播和点击路径。用户主动问“在哪”“怎么开”“点哪里”或“入口在哪”时，也立即触发这条门禁；不得先解释、查文档或让用户自己找。

发送时必须同时满足：

1. 图片用 Markdown 图片语法直接嵌入聊天，不得只发文件路径、普通链接或“见附件”。
2. 图片单独占一行，并放在点击说明之前。
3. Windows 路径改用正斜杠，并在盘符前保留 `/`，例如 `![Codex 的 Goal 入口](/C:/Users/NAME/.agents/skills/bravecow-onboarding/assets/ui/codex/08-goal-mode.png)`；必须把 `NAME` 换成当前真实路径。
4. 如果当前应用不渲染本地 Markdown 图片，改用应用已有的图片或附件发送能力。两种方式都失败时，明确告诉用户图片没显示并停在这里，不得无图继续教点击。
5. 看到聊天中真正出现图片后，才算通过门禁。文件存在不等于已经发给用户。

每一个需要操作界面的步骤都使用固定顺序：`图片 → 一句话点击动作 → 结果确认`。图片必须是回复的第一项，不先铺垫原理，也不先问用户按钮可能在哪。用户只需要照着红圈点；老师只能在点击后询问预期界面是否出现，不能把寻找按钮当成练习题。

如果一个操作有三次点击，就拆成三轮，每轮各发一张只圈当前目标的图。不得发第一张图后让用户自行猜后面的按钮。

## 视觉语言

- **真实操作图**：保留应用原始深色界面、目标周边最小上下文和一个红色主标记；不加装饰标题、箭头或第二强调点。
- **概念卡**：使用炭黑底、柔灰面板、白字与克制红色强调；不得画成可点击控件，不使用应用徽标冒充官方界面。
- **信息层级**：标题只表达一个结论；正文最多 5 个节点；优先横向 16:9，保证聊天框中不放大也能读清。
- **教学节奏**：UI 课必须显示一张最相关图片；非 UI 课才允许只用文字。不做封面、地图或图片轮播。
- **可访问性**：不只靠颜色传达含义；红圈必须同时包围清晰文本或图标；中文正文避免低对比度细字。
- **隐私**：打码块不得保留文字轮廓；账号头像、时间、路径、通知和旁支任务一并裁掉或完全遮挡。
...
课程不是固定 12 课。每个动态模块最多显示一张图：需要用户点击时优先显示当前平台实拍红圈图；没有对应按钮时才显示概念卡。

| 模块 | Codex | ZCode |
|---|---|---|
| 新任务 | C01 | Z01 |
| 任务列表/上下文 | C02 | Z02 |
| 项目/工作区 | C03；后续步骤按需要改用 C04 或 C05 | Z03；远程环境改用 Z04 |
| 需求四件套 | S04 | S04 |
| 计划 | C06 | Z05 |
| 版本/权限 | S06；需要调整权限时改用 C07 | S06；需要调整权限时改用 Z06 |
| Goal | C08 | Z07 |
| 模型 | C09 | Z08 |
| 思考策略 | C10 | Z09 |
| 材料/工具 | C11；需要桌面操作时改用 C12 | Z10；需要命令入口时改用 Z11 |
| Harness/Skills/插件 | S11；需要点击时改用 C13 或 C14 | S11；需要点击时改用 Z12 或 Z13 |
| 项目路线真实验收 | C15 | Z14；若已有安全文件链接，现场生成对应红圈图 |
| 原理路线真实验收 | 现场显示中性练习文件链接 | 现场显示中性练习文件链接；无安全链接时使用 Z14 |
...
- 任取“按钮在哪”类问题，回复中必须先出现可见图片，再出现点击说明；只有路径或文字说明判为失败。
- 任取一个界面操作步骤，回复第一项必须是当前点击目标图片；若先提问、先解释或要求用户找按钮，判为失败。
