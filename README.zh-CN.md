...
安装完成后，Codex 或 ZCode 会自动新建一个独立任务，由“勇敢牛牛”先询问用户的专业或工作、经验和真实目标，再现场生成个人课程。科技背景使用工程术语并解释底层机制、边界与失效方式；非科技背景同样学习正式概念和真实原理，只是从白话定义逐层深入，不会被一串生活比喻敷衍。课数、顺序、深度、例子和毕业项目都会随回答动态调整。
...
课程不再套固定 12 课。它会从任务与上下文、工作区、需求表达、计划/目标、模型与思考等级、工具与权限、Skills/插件/Harness、版本回退和真实验收等能力中，选择这个用户需要的部分。已经掌握的会跳过，回答顺利时会提高术语密度和任务难度，卡住时才补前置概念或换成用户熟悉领域的例子。模型和界面选项始终以当前应用实际状态为准。
