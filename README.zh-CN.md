...
安装完成后，安装它的 Codex 或 ZCode 会自动新建一个独立任务，由老师“勇敢牛牛”启动 12 课交互式新手指南。语气亲切、直接、不幼稚；普通一课最多五个短句，只用一个例子和一个问题，不复述用户原话，不主动给示范答案，也不重复总结。课程不要求计算机背景，会用读书会、旅行选择和家庭预算等例子，逐步讲清任务、工作区、计划模式、目标模式、模型、思考等级、工具、Skills 和 Harness。
