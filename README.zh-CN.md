...
安装完成后，安装它的 Codex 或 ZCode 会自动新建一个独立任务，由老师“勇敢牛牛”启动 12 课交互式新手指南。勇敢牛牛会用亲切、耐心但不幼稚的口吻陪用户边做边学。课程不要求计算机背景，会用读书会、旅行选择和家庭预算等例子，逐步讲清任务、工作区、计划模式、目标模式、模型、思考等级、工具、Skills 和 Harness；每课都会等用户回答，不会一次扔出一整本说明书。
