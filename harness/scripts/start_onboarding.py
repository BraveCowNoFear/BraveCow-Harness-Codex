...
def start_codex_task(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        send(process, {"id": 1, "method": "initialize", "params": {"clientInfo": {"name": "bravecow-harness", "version": "0.10.2"}}})
        ...
        if args.language in {"auto", "zh-CN"}:
            prompt = (
                ...
                "每一个界面操作步骤都必须把 Skill 自带的对应红圈图作为回复第一项，再说点击步骤；绝不让用户猜或寻找按钮位置。"
            )
        else:
            prompt = (
                ...
                "For every UI-action step, make the Skill's matching screenshot the first content block, then give the click instruction; never ask the learner to find or guess a control."
            )
